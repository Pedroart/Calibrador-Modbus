...
export function AdjustOffsetModal({ point, onSave, onClose }: AdjustOffsetModalProps) {
  const [offset, setOffset] = useState(point.offset)
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 max-w-sm w-full mx-4 shadow-lg">
        <h2 className="text-lg font-bold text-slate-100 mb-4">Ajustar Offset - {point.name}</h2>

        <div className="space-y-4">
          {/* Info actual */}
          <div className="bg-slate-900/50 border border-slate-700 rounded p-3 text-sm">
            <p className="text-slate-400">
              Valor actual: <span className="text-blue-400 font-semibold">{point.final_value.toFixed(2)}</span>
            </p>
            <p className="text-slate-400 mt-1">
              Offset actual: <span className="text-blue-400 font-semibold">{point.offset.toFixed(2)}</span>
            </p>
          </div>
          ...
          <div className="bg-slate-900/50 border border-slate-700 rounded p-3 text-sm">
            <p className="text-slate-400">
              Valor final:{" "}
              <span className="text-blue-400 font-semibold">{(point.final_value + offset).toFixed(2)}</span>
            </p>
          </div>
        </div>
